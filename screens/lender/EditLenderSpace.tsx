...
const EditLenderSpace: React.FC = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { parkingSpace } = route.params as { parkingSpace: any };

  const [title, setTitle] = useState(parkingSpace.title || '');
  const [price, setPrice] = useState(parkingSpace.price?.toString() || '');
  const [capacity, setCapacity] = useState(parkingSpace.capacity?.toString() || '');
  const [vehicleTypesAllowed, setVehicleTypesAllowed] = useState<VehicleType[]>(parkingSpace.vehicle_types_allowed || []);
  const [photos, setPhotos] = useState<PhotoState[]>(parkingSpace.photos?.map((photo: string) => ({ uri: photo, base64: photo })) || []);
    ...
    const fetchLocation = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .eq('parking_space_id', parkingSpace.id)
          .single();
  ...
  const uploadDocument = async (parkingSpaceId: string): Promise<string | null> => {
    ...
    try {
      ...
      // First, fetch the existing document details
      ...
      // If an existing document exists, delete it from storage and database
      if (existingDocuments && existingDocuments.document_url) {
        try {
          // Extract filename from the existing URL
          const existingFileName = existingDocuments.document_url.split('/').pop();
          
          if (existingFileName) {
            // Delete from storage
            await supabase.storage
              .from('spot_ownership')
              .remove([`${parkingSpaceId}/${existingFileName}`]);
          ...
          console.error('Error deleting existing document:', deletionError);
          // Non-critical error, so we'll continue with upload
        }
      }
  
      // Insert new document record into parking_space_documents table
  ...
  const handleUpdateSpot = async () => {
    ...
    try {
      ...
      const { error: parkingSpaceError } = await supabase
        .from('parking_spaces')
        .update({
          title: title.trim(),
      ...
      if (parkingSpaceError) {
        throw new Error(parkingSpaceError.message);
      }
      ...
      // Update document if a new one was uploaded
      if (ownershipDocument) {
        try {
      ...
      Alert.alert(
        "Success", 
        "Parking spot updated successfully. Your changes will be verified shortly.",
...
export default EditLenderSpace;